...
# Function to perform grid_search
from sklearn.model_selection import GridSearchCV
from sklearn.metrics import confusion_matrix, accuracy_score
def grid_search(classifier_pipline,
                features,
                target,
                parameter_grid,
                scoring,
    ...
    gs = GridSearchCV(classifier_pipline,
                      parameter_grid,
                      cv = n_folds,
                      scoring = scoring,
                      verbose = verbose)
    ...
    # Predict in sample
    y_hat_in = gs.predict(features)
    y_hat_in_proba = gs.predict_proba(features)
    best_model = gs.best_estimator_

    # Confusion Matrix
    class_labels = ["Dead","Alive"]
    cm =  confusion_matrix(y_pred=y_hat_in,
                           y_true=target,
                           labels=[1,0])
    ...
    return {'best_params': gs.best_params_,
            ...
            'y_hat': y_hat_in,
            'y_hat_in_proba': y_hat_in_proba,
            'best_model': best_model,
            'accuracy' : round(accuracy_score(target,y_hat_in),5),
            'roc_auc': round(roc_auc_score(target, y_hat_in_proba[:,1]),5)
            }

def predict_test(X_train, y, X_test, model_scores, model_name):
    features = X_train.columns.tolist()
    model = model_scores['best_model']
    model.fit(X_train, y)
    y_hat_test = model.predict_proba(X_test[features])
    test_predictions_submit = pd.DataFrame({"icustay_id": X_test["icustay_id"], "HOSPITAL_EXPIRE_FLAG": y_hat_test[:,1]})
    test_predictions_submit.to_csv(f'{model_name}_submission.csv', index = False)


from sklearn.model_selection import KFold
def oof_icd9_encoder(
    train_df,
    test_df,
    comorbidity_df,
    ...
    return train_features, test_features, icd_mortality_global